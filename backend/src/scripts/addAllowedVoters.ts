...
const main = async () => {
    ...
    if (!fileName || !studentNumberColHeader) {
        console.log("Usage: tsx addAllowedVoters.ts <filename.csv> <student number column header>");
        process.exit(1);
    }
    ...
    try {
        ...
        const newVotingStatuses = await Promise.all(
            data.map(async (row, idx) => {
                ...
                console.log(`Checking if student #${studentNumber} already has a voting status entry...`);
                if ((await VotingStatus.exists({ studentNumber })) !== null) {
                    console.warn(`Student #${studentNumber} already has a voting status, ignoring...`);
                    return null;
                }

                console.log(`Student #${studentNumber} is new, queueing to add later...`);
        ...
        const filteredVotingStatuses = newVotingStatuses.filter((val) => val !== null);

        if (filteredVotingStatuses.length > 0) {
            await VotingStatus.insertMany(newVotingStatuses);
        }
        console.log(
            `Successfully added ${filteredVotingStatuses.length} new voting statuses, ignored ${
